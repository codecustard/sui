...
persistent actor SuiPOC {
  ...
  /// Debug function to test BCS transaction format via dry run
  public func debugDryRunTransaction(
    recipientAddress : Text,
    amount : Nat64,
    gasBudget : Nat64
  ) : async Result.Result<Text, Text> {
    ...
    switch (await generateAddress(null)) {
      case (#err(error)) { return #err("Failed to get address: " # error) };
      case (#ok(wallet)) {
        // Get coins for this address
        switch (await SuiTransfer.getSuiCoins(rpcUrl, wallet.address)) {
          ...
          case (#ok(coins)) {
            ...
            await SuiTransfer.dryRunTransaction(
              rpcUrl,
              wallet.address,
              coin.coinObjectId,
              recipientAddress,
  ...
  /// Compare our BCS format with the working RPC txBytes
  public func debugCompareBcs(
    recipientAddress : Text,
    amount : Nat64,
    gasBudget : Nat64
  ) : async Result.Result<Text, Text> {
      ...
      case (#ok(wallet)) {
        switch (await SuiTransfer.getSuiCoins(rpcUrl, wallet.address)) {
          ...
          case (#ok(coins)) {
            ...
            await SuiTransfer.compareBcsWithRpc(
              rpcUrl,
              wallet.address,
              coin.coinObjectId,
              recipientAddress,
